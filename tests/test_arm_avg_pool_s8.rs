...
mod tests {
    ...
    use cmsis_nn_rs::{backend::{CmsisNN, CmsisNNDevice}, *};
    ...
    fn test_avgpool_i8() {
        ...
        let shape_x = Shape::new([AVGPOOLING_BATCH_SIZE, AVGPOOLING_INPUT_C, AVGPOOLING_INPUT_H, AVGPOOLING_INPUT_W]);
    ...
    fn test_avgpool_s8() {
        ...
        let ctx_size = unsafe { arm_avgpool_s8_get_buffer_size(AVGPOOLING_OUTPUT_W as i32, AVGPOOLING_INPUT_C as i32) };
        let mut ctx = cmsis_nn_context {
            size: ctx_size,
            buf: unsafe { HEAP.alloc(
        ...
        if !ctx.buf.is_null() {
            ctx.size = 0;
            unsafe {
                HEAP.dealloc(ctx.buf as *mut u8, 
                    Layout::from_size_align(ctx_size as usize, 1).unwrap()
                );
            }
        };
